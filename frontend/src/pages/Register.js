...
const Register = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!firstName || !lastName || !email || !username || !password) {
      ...
      setErrorType('all'); // Use 'all' as the error type for missing fields
      return;
    }
    ...
    try {
      ...
      if (error.response && error.response.data.error) {
        ...
        setErrorType(error.response.data.error.toLowerCase().includes('email') ? 'email' : 'username');
      } else {
        setMessage('Error creating account. Please try again.');
      }
    }
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-gray-900">
      <div className="bg-gray-800 p-8 rounded-lg shadow-2xl w-full max-w-md space-y-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="relative">
            ...
            <input
              ...
              onChange={(e) => setEmail(e.target.value)}
              placeholder="Email"
              className={`w-full pl-10 p-3 border rounded-lg bg-gray-700 text-white focus:outline-none ${
                errorType === 'email' ? 'border-red-500' : 'border-gray-600'
              }`}
            />
          </div>
          <div className="relative">
            ...
            <input
              ...
              placeholder="Username"
              className={`w-full pl-10 p-3 border rounded-lg bg-gray-700 text-white focus:outline-none ${
                errorType === 'username' ? 'border-red-500' : 'border-gray-600'
              }`}
            />
          </div>
          <div className="relative">
        ...
        {message && <p className={`mt-4 text-center ${errorType ? 'text-red-500' : 'text-green-500'}`}>{message}</p>}
        <p className="text-gray-400 text-center">
          Already registered?{' '}
          <button
            onClick={() => navigate('/login')}
