...
class PdfAPIView(APIView):
    def post(self, request, *args, **kwargs):
        ...
        uploaded_pdf = UploadedPDF(file=pdf_file, user=request.user)
        uploaded_pdf.save()
...
# API view for chatting with the uploaded PDF
class PdfChat(APIView):
    def get(self, request, *args, **kwargs):
        if not request.user or not request.user.is_authenticated:
            return Response({'error': 'User is not authenticated'}, status=status.HTTP_401_UNAUTHORIZED)
        ...
        if not pdf_id or not question:
            return Response({'error': 'Both PDF ID and question must be provided'}, status=status.HTTP_400_BAD_REQUEST)

        try:
            pdf_record = get_object_or_404(UploadedPDF, id=pdf_id, user=request.user)
            ...
            logger.error(f"Error during PDF chat: {e}")
